...
parser.add_argument(
    "--ckpt", type=str, default="ckpt/vctk_voicing_large_bs/epoch-124.ckpt"
)
parser.add_argument(
    "--config", type=str, default="ckpt/vctk_voicing_large_bs/config.json"
)
...
def load(src_wav):
    ...
    assert (
        sr == 16000
    ), "The source and target speech are not in 16k, adjust the code here to resample"
...
for line in tqdm(lines):
    ...
    with torch.no_grad():
        ...
        transfer["pitch-energy"] = model(
            UL, tgt_attributes["a_p"], src_attributes["a_s"], L=None, direct_UL=True
        )
        transfer["speaker"] = model(
            UL, src_attributes["a_p"], tgt_attributes["a_s"], L=None, direct_UL=True
        )
        transfer["rhythm"] = model(
            U, src_attributes["a_p"], src_attributes["a_s"], tgt_attributes["a_r"]
        )
        transfer["prosody"] = model(
            U, tgt_attributes["a_p"], src_attributes["a_s"], tgt_attributes["a_r"]
        )
