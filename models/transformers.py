...
class MultiheadAttention(nn.Module):
    ...
    def reshape(self, x):
        x = (
            x.view(x.size(0), x.size(1), self.nhead, self.head_dim)
            .transpose(1, 2)
            .contiguous()
        )
    ...
    def forward(
        ...
        if attn_bias is not None:
            assert attn_bias.size() == (
                ...
                q.size(1),
                k.size(1),
            ), f"Should be {(self.nhead, q.size(1), k.size(1))}. Got {attn_bias.size()}"
        ...
        if attn_mask is not None:
            assert attn_mask.size() == (
                q.size(1),
                k.size(1),
            ), f"Should be {(q.size(1), k.size(1))}. Got {attn_mask.size()}"
        ...
        if key_padding_mask is not None:
            assert key_padding_mask.size() == (
                batch_size,
                k.size(1),
            ), f"Should be {(batch_size, k.size(1))}. Got {key_padding_mask.size()}"
            ...
            key_padding_mask = key_padding_mask.reshape(
                batch_size * self.nhead, 1, k.size(1)
            )
        ...
        attn_weights = F.softmax(
            attn_weights * self.softmax_temp, dim=-1, dtype=attn_weights.dtype
        )
        ...
        attn_output = attn_output.transpose(1, 2).reshape(
            batch_size, q.size(1), self.d_model
        )
