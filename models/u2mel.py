...
class Generator(nn.Module):
    def __init__(self, hp):
        ...
        self.layers1 = nn.ModuleList(
            [
                ConvLNBlock(hp.hidden_size, hp.dropout, dilation=(2 * i + 1))
                for i in range(hp.mel_layers)
            ]
        )
...
class EGenerator(nn.Module):
    def __init__(self, hp):
        ...
        self.layers1 = nn.ModuleList(
            [
                ConvLNBlock(hp.hidden_size, hp.dropout, dilation=(2 * i + 1))
                for i in range(hp.mel_layers // 2)
            ]
        )
...
class Unit2Mel(nn.Module):
    def __init__(self, hp):
        ...
        self.spkr_unit_linear = nn.Linear(
            hp.hidden_size + hp.feature_size, hp.hidden_size
        )
        self.spkr_f0_linear = nn.Linear(
            hp.hidden_size + hp.feature_size, hp.hidden_size
        )
