...
from models.u2mel import Discriminator, Unit2Mel
...
class BaseTrainer(pl.LightningModule):
    ...
    def create_optimizer(self, parameters):
        optimizer_adam = optim.Adam(
            parameters, lr=self.hp.lr, betas=(self.hp.adam_beta1, self.hp.adam_beta2)
        )
...
class RVectorTrainer(BaseTrainer):
    ...
    def generator_parameters(self):
        return (
            list(self.RVEncoder.parameters())
            + list(self.dp.parameters())
            + list(self.f0p.parameters())
            + list(self.u2m.parameters())
            + list(self.embedding.parameters())
            + list(self.vp.parameters())
            + list(self.Ep.parameters())
        )
    ...
    def forward(self, batch):
        ...
        rv, speaker_embedding_real, dur, voiced = self.get_rv_spkr(
            batch["audio"], batch["audio_mask"]
        )
        rv, speaker_embedding, dur, voiced = self.get_rv_spkr(
            pitch_shifted_audio, batch["audio_mask"]
        )
        # Embed units
        dedup_unit = self.embedding(batch["dedup_unit"])
        unit = self.embedding(batch["unit"])
        pred_mels, duration_preds, f0, voiced_preds, E = self.run(
            dedup_unit,
            unit,
            rv,
            speaker_embedding,
            speaker_embedding_real,
            dur,
            voiced,
            batch,
        )
        # Duration prediction
        dur_loss = F.mse_loss(
            duration_preds[~batch["dedup_unit_mask"]],
            torch.log(1 + batch["duration"][~batch["dedup_unit_mask"]].float()),
        )
        # Pitch prediction
        f0_mask = batch["voiced"].bool()
        f0_loss = self.bce_loss(f0[f0_mask], batch["f0"][f0_mask])
        # Voiced prediction
        voiced_loss = self.bce_loss(
            voiced_preds[~batch["unit_mask"]],
            batch["voiced"][~batch["unit_mask"]].float(),
        )
        # MelSpec prediction
        mel_loss = F.l1_loss(
            pred_mels[~batch["mel_mask"]], batch["mel"][~batch["mel_mask"]]
        )
        # Energy prediction
        E_loss = self.bce_loss(
            E[~batch["mel_mask"]], batch["energy"][~batch["mel_mask"]]
        )
        # Embeddings
        ...
        embed_loss = F.mse_loss(E_embed.detach(), E_embed_pred) + F.mse_loss(
            F_embed.detach(), F_embed_pred
        )
        mel_loss += embed_loss
        # GAN Loss
    ...
    def infer(
        self, batch, shuffled_audio=None, shuffled_mask=None, shuffle_target=None
    ):
        rv, speaker_embedding, dur, voiced = self.get_rv_spkr(
            batch["audio"], batch["audio_mask"]
        )
        if shuffled_audio is not None:
            if shuffle_target == "rv":
                ...
                _, speaker_embedding, _, _ = self.get_rv_spkr(
                    shuffled_audio, shuffled_mask
                )
        # Embed units
        dedup_unit = self.embedding(batch["dedup_unit"])
        # Duration prediction
        ...
        duration_preds = torch.clamp(
            duration_preds, min=1.0
        )  # At least one frame is allocated(?)
        ...
        pred_mask = F.interpolate(
            (~unit_mask).float().unsqueeze(1), size=length
        )  # TTTT F
    ...
    def training_step(self, batch, batch_idx):
        ...
        E_loss, dur_loss, mel_loss, f0_loss, voiced_loss, gan_loss_g, gan_loss_d = self(
            batch
        )
    ...
    def synthesize(self, batch, shuffle_idx, target):
        vc_mel, mel_mask = self.infer(
            batch, batch["audio"][shuffle_idx], batch["audio_mask"][shuffle_idx], target
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx < self.hp.sample_num:
            ...
            voc = self.vocoder(
                batch["mel"][0][~batch["mel_mask"][0]].unsqueeze(0)
            ).float()
            shuffle_idx = torch.randperm(
                batch["audio"].size(0), device=batch["audio"].device
            )
            ...
            sw.add_audio(
                f"original/{batch_idx}", original_audio, self.global_step, 16000
            )
            sw.add_audio(
                f"condition/{batch_idx}",
                batch["audio"][shuffle_idx[0]][~batch["audio_mask"][shuffle_idx[0]]],
                self.global_step,
                16000,
            )
            sw.add_audio(
                f"swap_rv/{batch_idx}", swap_rv, self.global_step, self.hp.sample_rate
            )
            sw.add_audio(
                f"swap_speaker/{batch_idx}",
                swap_spk,
                self.global_step,
                self.hp.sample_rate,
            )
            sw.add_audio(
                f"reconstruct/{batch_idx}", rec, self.global_step, self.hp.sample_rate
            )
            sw.add_audio(
                f"vocoder/{batch_idx}", voc, self.global_step, self.hp.sample_rate
            )
