...
def StandardSampler(dataset, shuffle, distributed=False, world_size=None, rank=None):
    if distributed:
...
class DistributedRandomBucketSampler(data.Sampler):
    def __init__(
        self, nbuckets, length, batch_size, drop_last, num_replicas, rank, seed=1234
    ):
