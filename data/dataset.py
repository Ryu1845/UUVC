...
class SpeechDataset(data.Dataset):
    def __init__(self, hp, metadata):
        ...
        self.lengths = [os.path.getsize(f) / (16000.0 * 4) for f in self.data]
        bin_size = (self.hp.f0_max - self.hp.f0_min) / self.hp.pitch_bins
        self.f0_bins = (
            torch.arange(self.hp.pitch_bins, dtype=torch.float32) * bin_size
            + self.hp.f0_var_min
        )
    ...
    def seqCollate(self, batch):
        ...
        for audio, unit, dedup_unit, duration, mel, E, f0, voiced in batch:
            ...
            audio_mask = torch.BoolTensor(
                [False] * len(audio) + [True] * (m_a - len(audio))
            )
            audio = F.pad(audio, [0, m_a - len(audio)])
            # Deal with units
            unit_mask = torch.BoolTensor(
                [False] * len(unit) + [True] * (m_u - len(unit))
            )
            unit = F.pad(unit, [0, m_u - len(unit)], value=self.hp.vocab_size)
            # Deal with deduplicated units
            dedup_unit_mask = torch.BoolTensor(
                [False] * len(dedup_unit) + [True] * (m_d - len(dedup_unit))
            )
            dedup_unit = F.pad(
                dedup_unit, [0, m_d - len(dedup_unit)], value=self.hp.vocab_size
            )
    ...
    def __getitem__(self, i):
        ...
        energy, f0 = torch.FloatTensor(np.load(self.energy[i])), torch.FloatTensor(
            np.load(self.f0[i])
        )
        voiced = f0 != 0
        ...
        f0[~voiced] = -1000
        f0 = torch.exp(
            -((self.f0_bins.repeat(f0.size(0), 1) - f0.unsqueeze(-1)) ** 2)
            / (2 * self.hp.f0_blur_sigma**2)
        )
        energy = torch.exp(
            -((self.E_bins.repeat(energy.size(0), 1) - energy.unsqueeze(-1)) ** 2)
            / (2 * self.hp.E_blur_sigma**2)
        )
